...
auto get_process_id(std::string process_name) -> HANDLE
{
	...
	do
	{
		...
		RtlUnicodeStringToAnsiString(&image_name_astr, &it->ImageName, FALSE);
...
auto open_process(HANDLE pid, HANDLE& handle) -> void
{
	...
	NtOpenProcess(&handle, PROCESS_ALL_ACCESS, &attr, &client_id);
}
...
auto inject(HANDLE process, std::string dll) -> void
{
	...
	NtWriteVirtualMemory(process, allocated_memory, dll_name, sizeof(dll_name), nullptr);
	CreateRemoteThread(process, 0, 0, (LPTHREAD_START_ROUTINE)LoadLibrary, allocated_memory, 0, 0);
}

auto main() -> int
{
	const auto dll_name = std::string("csgo-internal.dll");
	...
	const auto pid = get_process_id("csgo.exe");
